...
def test_FuturesStitcher_last_but_one_df_adjusted(FuturesStitcher_source, add_sticher):
    ...
    input_df = FuturesStitcher_source[last_but_one_contract]
...
def test_FuturesStitcher_third_from_last_df_adjusted(
    ...
    input_df = FuturesStitcher_source[third_from_last_contract]
    mid_point = int(len(input_df.index) / 2)
    datapoint_index = input_df.index[mid_point]
    datapoint_input = input_df.loc[datapoint_index]
    datapoint_output = add_sticher.data.loc[datapoint_index]
...
def test_FuturesStitcher_first_df_cummulative_adjustment_correct(
    ...
    first_input_df = FuturesStitcher_source[first_contract]
...
def test_resulting_df_no_index_duplicates(add_sticher):
    assert add_sticher.data[add_sticher.data.index.duplicated()].empty
...
def test_first_df_starts_at_start_date(add_sticher):
    ranges = add_sticher._selector.date_ranges
    first_contract = list(ranges)[0]
    start_date = ranges[first_contract][0].replace(tzinfo=add_sticher.tz_info)
    first_cs = add_sticher._contract_slices[0]
    assert first_cs.df.index[0] >= start_date - add_sticher._buffer_offset


def test_inspect_returns_dataframe(add_sticher):
    result = add_sticher.inspect()
    assert isinstance(result, pd.DataFrame)
    assert not result.empty


def test_inspect_returns_none_when_no_rolls(FuturesStitcher_source):
    # use only first contract — no rolls possible, _roll_records is empty
    contracts = sorted(
        list(FuturesStitcher_source),
        key=lambda x: datetime.strptime(x.lastTradeDateOrContractMonth, "%Y%m%d"),
    )
    single_source = {contracts[0]: FuturesStitcher_source[contracts[0]]}
    sticher = FuturesStitcher(single_source)
...
def test_inspect_row_count(add_sticher):
    assert len(add_sticher.inspect()) == len(add_sticher._contract_slices) - 1


def test_non_overlapping_raises_warning(FuturesStitcher_source):
    source = dict(FuturesStitcher_source)
    contracts = sorted(
        list(source),
        key=lambda x: datetime.strptime(x.lastTradeDateOrContractMonth, "%Y%m%d"),
    )
    source[contracts[-3]] = pd.DataFrame(
        {
            "close": [1.0, 2.0],
            "open": [1.0, 2.0],
            "high": [1.0, 2.0],
        ...
        index=pd.DatetimeIndex(["2020-01-01 09:00", "2020-01-01 10:00"], tz="UTC"),
    )
    sticher = FuturesStitcher(source, "add")
    with pytest.warns(UserWarning, match="NonOverLappingDfsError"):
        _ = sticher.data
...
def test_make_roll_timestamp_intraday_tz_aware(sticher_for_sync):
    ...
    assert ts == pd.Timestamp("2024-03-11 11:00", tz="US/Eastern")
...
def test_make_roll_timestamp_intraday_tz_naive(sticher_for_sync):
    ...
    ts = sticher_for_sync._make_roll_timestamp(roll_day, df)
    assert ts == pd.Timestamp("2024-03-11 11:00")
...
def test_sync_point_exact_match_tz_aware(make_intraday_df, sticher_for_sync):
    ...
    df0 = make_intraday_df(
        tz="US/Eastern", start="2024-03-11 11:00", periods=48, freq="1h"
    )
    df1 = make_intraday_df(
        tz="US/Eastern", start="2024-03-11 11:00", periods=48, freq="1h"
    )
    sync = sticher_for_sync._sync_point(df0, df1, roll_day)
    assert sync == pd.Timestamp("2024-03-12 11:00", tz="US/Eastern")


def test_sync_point_exact_match_tz_naive(make_intraday_df, sticher_for_sync):
    roll_day = datetime(2024, 3, 13)
    df0 = make_intraday_df(tz=None, start="2024-03-11 09:00", periods=48, freq="1h")
    df1 = make_intraday_df(tz=None, start="2024-03-11 09:00", periods=48, freq="1h")
    sync = sticher_for_sync._sync_point(df0, df1, roll_day)
    assert sync == pd.Timestamp("2024-03-12 11:00")
...
def test_sync_point_present_in_df0_but_not_df1_falls_back(sticher_for_sync):
    roll_day = datetime(2024, 3, 13)
    # roll_ts = 2024-03-12 11:00 Eastern (roll_day - 1 bday) = 15:00 UTC
    # df0 has 15:00 UTC, df1 skips it; last common point before roll_ts is 14:30
    df0 = pd.DataFrame(
        {"close": range(4)},
    ...
    sync = sticher_for_sync._sync_point(df0, df1, roll_day)
    assert sync == pd.Timestamp("2024-03-12 14:30", tz="UTC")
    assert sync in df0.index
    assert sync in df1.index
