...
def _swing(data: np.array, f: np.array, margin: np.array) -> np.array:
    ...
    Numpy array with shape [data.shape[0], 6], where columns have the
    following meaning for every row corresponding to price data:

    0 - state - (1) for upswing (-1) for downswing

    1 - extreme - current extreme against which filter is applied to
    determine if swing changed direction
    ...
    4 - max_list - latest upswing pivot

    5 - min_list - latest downswing pivot
    """
...
def swing(data: pd.DataFrame, f: Union[float, np.array, pd.Series],
          margin: Optional[float] = None) -> np.array:
    """
    ...
    f: filter to determine if swing changed direction
...
def stop_loss(df: pd.DataFrame,
    ...
    """
    User interface for stop-loss applying functions.  Convert a series
    with signals or positions into a series with positions resulting
    from applying a specified type of stop loss.
    ...
    df - input dataframe, must have following collumns: 'high', 'low',
    'distance', 'tp_distance' and either 'signal' or 'position' (if it
    contains both, 'signal' will be overridden with values extracted
    from 'positions' - whenever position value changes, that's a
    transaction signal)

    mode - stop loss type to apply, possible values: 'fixed', 'trail'

    tp_distance - take profit distance in price points or zero if none
    ...
    Position series after applying the stop loss.
    """
