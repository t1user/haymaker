...
class Candle(ABC):
    def __init__(
        self, streamer, contract_fields: Union[List[str], str] = "contract", **kwargs
    ):
    ...
    def save(self, saver):
        if self.df is not None:
            ...
            saver.save(self.streamer.all_bars_df, "all_bars", self.contract.localSymbol)
...
class SingleSignalMixin:
    def process(self: CandleMixin) -> None:
        log.debug(
            f"New candle for {self.contract.localSymbol} "
...
class FilterMixin:
    def filter(self, df: pd.DataFrame) -> pd.DataFrame:
        df["ema_fast"] = df.price.ewm(
            span=self.ema_fast, min_periods=int(self.ema_fast * 0.6)
        ).mean()
        df["ema_slow"] = df.price.ewm(
            span=self.ema_slow, min_periods=int(self.ema_slow * 0.6)
        ).mean()
        df["filter"] = np.sign(df["ema_fast"] - df["ema_slow"])
        df["filtered_signal"] = df["signal"] * ((df["signal"] * df["filter"]) == 1)
        return df
...
class CarverCandle(SingleSignalMixin, Candle):
    def get_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        df["carver"] = indicators.carver(df.price, self.periods).rolling(15).mean()
...
class BreakoutRsiCandle(SingleSignalMixin, Candle):
    def get_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # breakout signal
        df["breakout_s"] = indicators.min_max_signal(df.price, self.periods)
        # rsi signal
        df["rsi"] = indicators.rsi(df.price, self.rsi_periods)
        df["carver_rsi"] = (
            indicators.carver(df["rsi"], 100).rolling(self.rsi_smooth).mean()
        )
        df["rsi_s"] = indicators.range_crosser(df["carver_rsi"], self.rsi_threshold)
        # combined signal
...
class BreakoutLockCandle(SingleSignalMixin, Candle):
    def get_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["lock"] = (
            (
                (
                    df.pre_lock.shift().rolling(self.lock_periods).min()
...
class BreakoutLockBufferCandle(SingleSignalMixin, Candle):
    def get_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["atr"] = indicators.atr(df, self.atr_periods)
        df["signal"] = indicators.min_max_buffer_signal(df.price, self.periods, df.atr)
        df["filter"] = np.sign(df["ema_fast"] - df["ema_slow"])
        df["pre_lock"] = df["signal"] * ((df["signal"] * df["filter"]) == 1)
...
class BollingerCandle(DoubleSignalMixin, Candle):
    ...
    def get_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["close_signal"] = ((df["direction"] * df["direction"].shift()) < 0) * df[
            "direction"
        ]
        return df
...
class BreakoutStrenthFilteredCandle(SingleSignalMixin, Candle):
    def get_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["strength_filter"] = df["strength_slow"] >= df["strength_slow_median"]

        df["signal"] = indicators.min_max_signal(df.price, self.periods)

        df["pre_strength"] = df["signal"] * ((df["signal"] * df["ema_filter"]) == 1)
...
class DonchianCandle(DoubleSignalMixin, Candle):
    def get_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["signal"] = indicators.min_max_signal(df.price, self.periods)
        df["filtered_signal"] = df["signal"] * ((df["signal"] * df["ema_filter"]) == 1)

        df["close_signal"] = indicators.min_max_signal(df.price, self.close_periods)
