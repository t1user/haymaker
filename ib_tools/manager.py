...
from ib_tools.portfolio import Portfolio
from ib_tools.saver import AbstractBaseSaver, PickleSaver
...
class Manager:
    ...
    def connect_candles(self, now):
        for candle in self.candles:
            ...
            candle.entrySignal.connect(self.portfolio.onEntry, keep_ref=self.keep_ref)
            candle.closeSignal.connect(self.portfolio.onClose, keep_ref=self.keep_ref)
            candle.signal.connect(self.portfolio.onSignal, keep_ref=self.keep_ref)
            candle.set_now(now)
            # run candle logic
            candle(self.ib)
    ...
    def __str__(self):
        return (
            f"Manager: ib: {self.ib}, candles: {self.candles}, "
