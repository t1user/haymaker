...
from haymaker.datastore import Store  # type: ignore
...
def get_data(contract, start_date=START_DATE, end_date=END_DATE):
    return store.read(symbol_dict[contract]).sort_index().loc[start_date:end_date]
...
def get_vol(data, vol_lookback):
    ...
    data["vol_returns"] = (
        data["returns"]
        .ewm(span=vol_lookback, min_periods=int(vol_lookback * 0.7))
        .std()
    )
...
def calibrate(
    ...
    if adjustment is not None:
        adjustments = pd.Series([adjustment] * len(inds.columns), index=inds.columns)
    else:
...
def _simulate(
    inds: pd.DataFrame, weights: pd.Series, adjustments: pd.Series, multiplier: float
) -> pd.DataFrame:

    scaled_inds = (inds * adjustments).clip(lower=-20, upper=20)
    scaled_inds_combined = (scaled_inds * weights).sum(axis=1)
    scaled_inds["forecast"] = (multiplier * scaled_inds_combined).clip(
        lower=-20, upper=20
    )
    return scaled_inds
...
def _data(
    ...
    if candle_volume == "fixed":
        ...
        candles = VolumeGrouper(data, dynamic=True, multiple=min_dict[contract]).df
    vols = get_vol(candles, vol_lookback).dropna()
    inds = pd.DataFrame([ind(candles, p, smooth, vols) for p in periods]).T.dropna()
...
def calibrate_multiple(
    ...
    data, candles, vols, inds = _data(
        contract, ind, periods, vol_lookback, start_date, end_date, time_int, smooth
    )
    ...
    return (
        pd.DataFrame(weights).T.mean(),
        pd.DataFrame(adjustments).T.mean(),
...
def run(
...
) -> Union[pd.DataFrame, Tuple[pd.DataFrame, Tuple[pd.Series, pd.Series, float]]]:
    ...
    cal_data_end = inds.resample("M").last().index[calibration_months] + timedelta(
        days=1
    )
    cal_inds = inds.loc[:cal_data_end]

    # calibrate
    weights, adjustments, multiplier, corr = calibrate(cal_inds, adjustment, multiplier)
