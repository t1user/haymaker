...
3. `Controller.run()` reads or initializes state, then `Controller.sync()` delegates to a sync coordinator. The coordinator validates broker position freshness, relinks current `ibi.Trade` objects to local records, runs order/position reconciliation against direct broker and state-machine reads, applies recovery through explicit appliers, and waits after broker-affecting remedies before later broker queries.
